...
func main() {
	...
	mySQLConnectionDataEstate = NewMySQLConnectionEnv("20.0.0.125")
	mySQLConnectionDataChair = NewMySQLConnectionEnv("20.0.0.115")
...
func initialize(c echo.Context) error {
	...
	if err := initSQL(*mySQLConnectionDataEstate, schema); err != nil {
		c.Logger().Errorf("Initialize script error schema estate: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	if err := initSQL(*mySQLConnectionDataChair, schema); err != nil {
		c.Logger().Errorf("Initialize script error schema chair: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}

	if err := initSQL(*mySQLConnectionDataEstate, estate); err != nil {
		c.Logger().Errorf("Initialize script error estate: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}

	if err := initSQL(*mySQLConnectionDataChair, chair); err != nil {
...
func initSQL(conn MySQLConnectionEnv, path string) error {
